...
{
    public List<List<char>> TestInstructions { get; } = "inputdata/day15_test.txt".Stream().GetChars().Split( ',' ).Select( chars => chars.ToList() ).ToList();
    public IEnumerable<IEnumerable<char>> ActualInstructions { get; } = "inputdata/day15.txt".Stream().GetChars().Split( ',' ).Select( chars => chars.ToList() ).ToList();
    public void Part1()
    {
