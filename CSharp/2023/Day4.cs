...
{
    public IEnumerable<LotteryCard> TestData = "2023/inputdata/day4_test.txt".GetLines().Select( LotteryCard.FromString ).ToList();
    public IEnumerable<LotteryCard> ActualData = "2023/inputdata/day4.txt".GetLines().Select( LotteryCard.FromString ).ToList();
...
public static partial class Helpers
{
    ...
    public static readonly Parser<LotteryCard> LotteryCard =
        Parse.String( "Card" ).Then( _ => Parse.Char( ' ' ).AtLeastOnce() )
            ...
            .Select( item => new LotteryCard( item.id, item.winners, item.yourNumbers ) );
